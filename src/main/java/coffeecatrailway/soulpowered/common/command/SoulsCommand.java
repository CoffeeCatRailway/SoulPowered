    ...
    private static int getValue(CommandContext<CommandSource> source, Entity entity)
    {
        if (entity instanceof LivingEntity)
        {
            entity.getCapability(SoulsCapability.SOULS_CAP).ifPresent(handler ->
                    source.getSource().sendSuccess(new TranslationTextComponent("commands." + SoulMod.MOD_ID + ".souls.get", entity.getDisplayName(), handler.getSouls()), true));
            return 1;
        }
        return 0;
    }
        ...
        if (canContinue)
        {
            ...
            for (Entity entity : entities)
            {
                ...
                if (cap.isPresent())
                {
                    ...
                    source.getSource().sendSuccess(new TranslationTextComponent("commands." + SoulMod.MOD_ID + ".souls.modify.set", entity.getDisplayName(), souls), true);
                    i++;
                }
            }
            return i;
            ...
            for (Entity entity : entities)
            {
                ...
                if (cap.isPresent())
                {
                    if (remove)
                    {
                        ...
                        source.getSource().sendSuccess(new TranslationTextComponent("commands." + SoulMod.MOD_ID + ".souls.modify.remove", entity.getDisplayName(), souls), true);
                    } else
                    {
                        cap.orElseThrow(NullPointerException::new).addSouls(souls, true);
                        source.getSource().sendSuccess(new TranslationTextComponent("commands." + SoulMod.MOD_ID + ".souls.modify.add", entity.getDisplayName(), souls), true);
                    }
                    i++;
                }
            }
